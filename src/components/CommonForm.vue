<template>
  <el-form :inline="inline" :model="form" ref="form" label-width="100px">
    <el-form-item v-for="item in fomrlabel" :key="item.model" :label="item.label">
      ...
      <el-select v-model="form[item.model]" placeholder="请选择" v-if="item.type === 'select'">
        <el-option v-for="item in item.opts" :key="item.val" :label="item.label" :value="item.value"></el-option>
      </el-select>
